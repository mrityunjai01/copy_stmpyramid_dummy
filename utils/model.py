...
class Node:
    ...
    def store_weight(self):
        ...
        if (self.path != None):
            ...
            directory = 'solver=' + str(self.sol_name) + '_Tree_Height=' + str(self.tree_height) + '_C=' + str(
                self.C) + '_rank=' + str(self.rank) + '_constrain=' + str(self.constrain) + '_wnorm=' + str(
                self.wnorm) + '_decomp=' + str(self.decomp) + '_tuneC=' + str(self.tuneC) + '_decomprank=' + str(
                self.decomprank)
    ...
    def dfs(self, narr, npos, x1, x2):
        ...
        if (self.decomp == True):
            wf = construct_W_from_decomp(wf, self.ogXtrainshape)
            if (narr.shape[1:] != wf.shape):
                narr = np.zeros(wf.shape)
                narr = narr[None, :, :]
        '''if len(shape) == 3:
    ...
    def change_ogshape(self, og):
        if (self == None):
            return
        self.ogXtrainshape = og
        if (self.A != None):
            self.A.change_ogshape(og)
        if (self.B != None):
            self.B.change_ogshape(og)
