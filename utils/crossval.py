...
class model_cv(ClassifierMixin):
    def __init__(self,indim,sol_name='STM',C = 1.0,rank = 3,wconst='maxmax',xa = None,xb = None,constrain = 'lax',
                 wnorm = 'L1',tree_height=3,path=None,tuneC=1e-6,decomp=False,decomprank=5):
        self.indim = indim
        ...
        self.model = Node(indim=self.indim,sol_name=self.sol_name,C=self.C,rank=self.rank,wconst=self.wconst,
                          ...
                          ogXtrainshape=self.indim,decomprank=self.decomprank)

    def fit(self,X,Y):
        if(self.decomp == True):
            X,ogXtrainshape = data_decomp(X,self.decomprank)
            self.model.change_ogshape(ogXtrainshape)
    ...
    def get_params(self,deep=True):
        return {"indim":self.indim,"sol_name":self.sol_name,"C":self.C,"rank":self.rank,"wconst":self.wconst,
                "constrain":self.constrain,"wnorm":self.wnorm,"tree_height":self.tree_height,"path":self.path,
                "decomprank":self.decomprank,"decomp":self.decomp,"tuneC":self.tuneC}
